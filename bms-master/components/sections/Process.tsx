                ...
                <div className="space-y-12">
                    {steps.map((step, idx) => (
                        <div key={idx} className={`relative flex flex-col md:flex-row gap-8 ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                            ...
                            {/* Timeline Dot */}
                            <div className="absolute left-4 md:left-1/2 top-0 w-8 h-8 -translate-x-1/2 bg-card border-2 border-primary rounded-full z-10 flex items-center justify-center">
                                <div className="w-2 h-2 bg-primary rounded-full"></div>
                            </div>

                            {/* Content */}
                            <div className="ml-12 md:ml-0 md:w-1/2 md:px-12">
                                <div className="bg-card shadow-md p-6 rounded-xl border border-border hover:border-primary/30 transition-colors">
                                    <div className="text-primary text-sm font-mono mb-2">0{idx + 1}</div>
